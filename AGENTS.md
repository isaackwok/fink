...
- **Qualification**: count ≥ 2 (candidate included), all directors counted (co-directors too), actor = stored top-10 billing; response carries stable keys (person id / genre id / ISO code / era year) + `count`, ordered server-side — the client never re-sorts.
