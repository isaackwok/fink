...
- **home/** - Main dashboard: journal entries list, empty state, add movie button
  ...
  - `widgets/` - JournalCard, JournalsList, EmptyPlaceholder, AddMovieButton.
    - `JournalCard`: long-press triggers iOS-style `CupertinoContextMenu` with Edit / Share / Delete actions, all delegating to `lib/features/journal/widgets/journal_actions.dart`. Tap navigation is gated on `animation.value == 0` to ignore taps during the context-menu zoom. The card is wrapped in a `ConstrainedBox(maxWidth: 200, maxHeight: 340)` to sidestep a `CupertinoContextMenu` layout assertion — do not remove. Visual spec: outer padding `fromLTRB(8, 8, 8, 12)`, 12px gap below poster, 8px gap between title and date, and the title+date pair gets an extra `EdgeInsets.symmetric(horizontal: 4)` inset. `JournalCard.completionPreview` renders a non-interactive 224px-wide variant without the context menu: 10/10/10/16 outer padding, a 204×268 poster, 16px poster-to-title gap, 10px title-to-date gap, and 6px inner text inset. The home-grid padding values are pinned by `journal_card_test.dart`'s "layout spec" group — updating them requires updating the test AND `nonPosterHeight` in `journals_list.dart`.
    - `JournalsList`: watch-month grouping + sorting (using `watchedAt`) live in `groupedJournalsProvider` (in `journals.dart`), a derived provider memoized per journals change — the widget just renders it. Grid uses a `LayoutBuilder` that computes `mainAxisExtent` from actual cell width, not `childAspectRatio`. If you change card padding / gap / text size / poster ratio, update `nonPosterHeight` and `posterAspectFactor` in `journals_list.dart` so cells stay tight. Current values: `nonPosterHeight = 70` (= 8 top pad + 12 image-title gap + 16 title (ceil of 14·1.1) + 8 title-date gap + 14 date (ceil of 12·1.1) + 12 bottom pad), `horizontalPaddingPerCard = 16` (= 8 × 2 sides). The value is intentionally tight — over-estimating leaves visible empty space below the date because `Flexible(fit: loose)` reserves the slot but the inner Column shrinks to its content.

- **journal/** - Core journaling features with full workflow from movie selection to saving
  - `controllers/` - `journal_state.dart` (SceneItem + JournalState model), `journal_mode.dart` (JournalMode enum + JournalModeNotifier), `journal.dart` (JournalController + provider; re-exports the other two, so `controllers/journal.dart` remains the one import for journal state), `journals.dart` (JournalsState list), `journal_insights.dart` (see [Journal insights](#journal-insights-achievements--emotion-echoes))
  - `data/` - `journal_insights_api.dart`: `Achievement` model (`kind`/`key`/`name`/`count`; `fromJson` returns null for unknown kinds so a Part 2 server emitting `festival` can't break this client) + `JournalInsightsApi` wrapping `functions.invoke('journal-insights')`, exposed via `journalInsightsApiProvider` (quesgen pattern) so tests fake it.
  - `screens/` - Journaling (main editor), JournalComplete (post-save success screen), JournalContent (view saved journal; `showShareAndDelete: false` hides the share button and the Delete menu item for callers outside the Home → JournalContent stack, e.g. emotion echoes), MoviePreview, ThoughtsScreen (`thoughts.dart`), CaptionEditor. Note `ThoughtsScreen` (screen) and `ThoughtsEditor` (widget, below) are different classes — don't confuse them.
  ...
  - `widgets/` - RatingSelector (10-heart rating), EmotionsSelectorButton, EmotionsSelectorBottomSheet, ScenesSelector (whose selected-scene card is `SelectedSceneCard`), ScenesSelectSheet (whose grid tile is `SceneGridTile`), SceneCard, ReviewItem, ReviewsBottomSheet (opened only via `ReviewsBottomSheet.show(context)` — ThoughtsScreen and ReviewsFloatingButton share it), ThoughtsEditor, AiReferencesAccordion, JournalContentMoreMenu, and `journal_actions.dart` — a set of shared helper functions (`editJournal`, `shareJournal`, `confirmDeleteJournal`, `deleteJournal`) that encapsulate the domain actions a journal can undergo. Reused by both the more-menu on `JournalContent` and the long-press menu on `JournalCard`. The helpers own the *domain action* (load state / navigate to editor, confirm dialog, Supabase delete + toast, navigate to `TicketPosterPickerScreen`) but intentionally leave post-action navigation (e.g. popping after delete) to the caller, since that depends on which screen initiated the action.
    - Journal-insights widgets: `AchievementsSection` ("Your achievements" header + a row-based 2-col grid whose odd trailing card stretches to the full row; `Skeletonizer.zone` while loading — note `Skeletonizer.zone` builds a *private* subclass, so tests must match `w is Skeletonizer`, not `byType`; hidden on error/resolved-empty), `AchievementCard` (kind icon → bold ordinal + grey connector → name resolved from the stable `key` via `tmdb_genres`/`country_names`/era labels, payload `name` for people), `EmotionEchoesSection` (header bolds the winning shared-emotion group, built from `headerEmotions` — never by splitting a localized sentence), `EmotionEchoCard` (Figma 7363:24722 / card 7487:9770, thought-less variant 7369:29163: centered AvenirNext **w700** title, `NothingYouCouldDo` watch date as `MMM, yyyy`, a 207px still of the first selected scene **only when the journal has one** — no TMDB backdrop fallback, a scene-less card simply has no image row — then the thoughts excerpt **centered between two `GoogleFonts.flavors` 36pt quote marks** in `#FFF1D7` — the only place that font/colour appears — capped at **5 lines** — `_fitExcerpt` measures with a `TextPainter` inside a `LayoutBuilder` and backs the cut off word by word until the text plus a plain "..." ellipsis fits (the ellipsis inherits the thoughts styling; the whole card opens the journal); a short text renders in full with no ellipsis; a thought-less journal drops the quote marks for the italic "No thoughts for this movie yet" line plus a bold "Fill in memory" + pencil row → `editJournal` straight into the editor). Card gaps are 12 title→date, 20 date→still, 12 still→quotes (16 still→nudge), 16 quote→text, and the excerpt sits 8pt below the marks' top; cards are 12 apart. Both insights headers sit 16pt above their cards; achievement rows are 132pt tall with 12pt gutters and the odd trailing card remains full-width. Tapping a card opens `JournalContent(showShareAndDelete: false)` — share and delete are hidden there because both assume the Home → JournalContent stack; Edit stays. Each section owns its own top spacing so a hidden section collapses entirely: achievements a plain 60pt gap (no divider — Figma 7363:24722), echoes a plain **60pt** gap (the old hairline divider is gone). The header mark is `EmotionEchoHeaderIcon`, a painted 24×24 pair of 70%-alpha circles tinted with the group colors of the first two `headerEmotions` via `EmotionGroupColors.of` — no SVG asset. Pinned by `emotion_echo_card_test.dart` / `emotion_echoes_section_test.dart`. `JournalCompleteScreen` is a top-anchored `SingleChildScrollView` column (79pt top inset: the block sits at y=133 on the 844pt frame, below its 54pt status bar), not a viewport-height centered slot. Its completion preview is followed by a 32pt gap, a 36pt Share Ticket button, a 16pt gap, and a 28pt View Journal link (rounded Figma sizing).
...
`JournalState.watchedAt` is the user-selected calendar day, normalized to local midnight and serialized as `yyyy-MM-dd`. Supabase stores it in nullable `journals.watched_at` (`date`), with INSERT/UPDATE column grants. Legacy NULL/missing values fall back to the local `createdAt` day; creation and modification timestamps remain audit fields. Do not run watch dates through UTC timestamp conversion. `WatchDateSelector` renders the Figma date + 4px gap + dropdown arrow and opens a localized calendar in both create and edit mode; confirmation calls `setWatchedAt`, cancellation leaves state untouched. Home cards/month grouping, journal view, completion preview, and emotion echoes use `watchedAt`. Apply `20260912000100_add_journal_watched_at.sql` before shipping the client.
...
3. **Journal Editing**:
   ...
   - In edit mode: ThoughtsScreen hides the sticky-bottom Reviews bar and "Add" card, review taps are no-ops, date uses the shared `WatchDateSelector`
...
### Journal insights (achievements & emotion echoes)

Shown once per journal on `JournalCompleteScreen`, right after saving (no per-journal metadata column, no backfill).

- **Achievements are computed server-side** by the `journal-insights` Edge Function (`supabase/functions/journal-insights/`): per dimension (director → actor → era → country → genre), how many DISTINCT films the user journaled share that value with the candidate. Keyed on `tmdb_id`, not `journal_id` — the candidate id is unioned into every per-dimension set whether or not the journal row exists yet, so the answer is identical before and after save. That idempotence is what makes the **prefetch** legal: `JournalingScreen.initState` (create mode only) fire-and-forgets the family future, and by save time the answer is cached. `JournalCompleteScreen` still calls `refresh()` post-frame (same-session staleness guard; Riverpod keeps the cached value visible during the refetch).
- **Era buckets split at 2020**: release year ≥ 2020 counts per year (key = year, shown as "2024"), earlier per decade (key = decade start, shown as "1990s"). Key ranges can't collide, so the client picks the label purely off `key >= 2020`. The split lives in the function's `buildIndex()`; the card's `_isYearBucket` mirrors it.
- **Qualification**: count ≥ 2 (candidate included), all directors counted (co-directors too), actor = stored top-10 billing; response carries stable keys (person id / genre id / ISO code / era year) + `count`, ordered server-side — the client never re-sorts. It does **dedupe**: `onePerKind()` (in `journal_insights.dart`, applied in the controller's fetch) keeps only the first achievement of each kind, so the section shows at most one card per dimension.
- `journalInsightsControllerProvider` = `AsyncNotifierProvider.family<…, List<Achievement>, int>` — copies `movieDetailControllerProvider`'s decisions verbatim (NOT autoDispose: prefetch-then-navigate survival; retry disabled: `.future` must settle so the section hides on error instead of hanging). Any failure → `AsyncError` → section hidden; the rest of the screen is unaffected.
- **`movie_facts` cache table** backs the function (see the `supabase-data` skill): service-role-only, lazily miss-filled from TMDB (`?append_to_response=credits`, concurrency 8, per-id tolerance). `scripts/warm_movie_facts.ts` is the idempotent one-time warmer for pre-existing journals — defense in depth, not a dependency.
- **Emotion echoes are pure client Dart** (`computeEmotionEchoes` in `journal_insights.dart`): a previous journal qualifies with ≥ 2 emotions shared with the just-saved one; journals group by their *exact* shared-emotion set; the largest set wins (a superset forms its own bigger group, so "most overlapping wins" falls out of the grouping); a size tie goes to the group with the highest average rating; the winning group's journals display in random order (injectable `Random` seam for tests), capped at 2. Same-movie journals and the journal itself are excluded; retired emotions match by id. `emotionEchoesProvider` (autoDispose family by journal id) derives from `journalsControllerProvider` — safe because `save()` awaits `refreshJournals()` before navigating.
- **Rollout order matters**: apply migration → `supabase functions deploy journal-insights` (needs the `TMDB_ACCESS_TOKEN` secret) → run `warm_movie_facts.ts` → ship the app build. An app shipped first degrades gracefully (invoke fails → section hidden).
- Display helpers: `lib/core/utils/tmdb_genres.dart` (19 fixed ids, en/zh-Hant), `country_names.dart` (curated ISO subset, raw-code fallback), `ordinal.dart` (en suffixes with the 11–13 exception; zh returns the bare digit because the ARB wraps it as 第 N 部 — gen_l10n has no `selectordinal`).
...
- Emotion colors are handled in the UI layer rather than the data model. The per-**group** accent (`Uplifting`/`Intense`/`Soothing`/`Quiet`/`Perspectives`) lives in `lib/features/emotion/emotion_group_colors.dart` (`EmotionGroupColors.of(group)`), shared by `EmotionsSelectorBottomSheet`'s section labels/chips and the echo header icon — don't re-declare those hexes. The energy-mix *gradients* stay private to `EmotionsSelectorButton`.
