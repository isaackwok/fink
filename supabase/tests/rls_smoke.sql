...
select ok(not has_table_privilege('authenticated','public.movie_facts','SELECT'),
          'authenticated cannot read movie_facts (function-only cache)');
select ok(not has_table_privilege('authenticated','public.movie_facts','INSERT'),
...
select ok((select relrowsecurity from pg_class where oid = 'public.movie_facts'::regclass),
          'movie_facts RLS enabled');
