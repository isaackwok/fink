...
void main() {
  final skipReason =
      Platform.environment['SUPABASE_TEST'] == '1'
          ? null
  ...
  group('SupabaseDbManager against a live stack', skip: skipReason, () {
    ...
    test('profile: create, read back, enforce username uniqueness', () async {
      ...
      expect(
        await db.usernameAvailable(username.toUpperCase()),
        isFalse,
    ...
    test(
      'journal: insert round-trips every field, including wall time',
      () async {
        final createdAt = Jiffy.parseFromDateTime(
          DateTime(2025, 9, 29, 19, 44, 2),
        );
    ...
    test('journal: update persists and preserves created_at', () async {
      ...
      final original = (await db.getJournalsCollection(
        userA,
      )).firstWhere((j) => j.id == id);
      ...
      final updated = (await db.getJournalsCollection(
        userA,
      )).firstWhere((j) => j.id == id);
    ...
    test('journal: bulk insert returns one id per journal', () async {
      ...
      final titles = (await db.getJournalsCollection(
        userA,
      )).map((j) => j.movieTitle);
    ...
    test('journal: delete removes the row', () async {
      ...
      expect(
        (await db.getJournalsCollection(userA)).map((j) => j.id),
        contains(id),
      );
    ...
    test(
      'RLS: a second user cannot read, modify, or delete the first\'s data',
      () async {
        final before = (await db.getJournalsCollection(
          userA,
        )).firstWhere((j) => true);
        ...
        expect(
          await clientB.from('journals').select().eq('user_id', userA),
          isEmpty,
        );
        expect(
          await clientB.from('profiles').select().eq('id', userA),
          isEmpty,
        );
        ...
        await clientB
            .from('journals')
            .update({'thoughts': 'hijacked'})
            .eq('id', victimId);
        await clientB.from('journals').delete().eq('id', victimId);

        final after = (await db.getJournalsCollection(
          userA,
        )).where((j) => j.id == victimId);
        expect(
          after,
          hasLength(1),
          reason: 'B must not be able to delete A\'s row',
        );
        expect(
          after.single.thoughts,
          before.thoughts,
