...
- `supabase/migrations/` — versioned schema (`profiles`, `journals`, `sync_tombstones`, `firebase_identity_map`, `movie_facts`), RLS on every table, tombstone triggers, and the `username_available` / `claim_migrated_data` RPCs. `supabase/tests/rls_smoke.sql` holds 72 pgTAP assertions; run with `supabase test db`.
...
- `supabase/functions/journal-insights/` — computes achievements for the app's journal-complete screen: per-dimension distinct-film counts over the caller's journals ∪ one candidate `tmdb_id`. Reads journals with the **user client** (RLS scopes rows naturally); reads/writes `movie_facts` with the admin client. Requires the `TMDB_ACCESS_TOKEN` secret (`supabase secrets set`). Deploy **before** shipping an app build that calls it; the client hides the section on error.
- **`movie_facts` is a function-only cache table** (TMDB facts: directors, top-10 cast, genre ids, countries, release year — near-immutable for released films; `fetched_at` enables a refresh policy later). It follows the `sync_tombstones` pattern — RLS enabled, zero policies, zero client grants — **plus explicit `grant select, insert, update … to service_role`**. That grant is not optional: the hardened default ACL (`20260725033216`) means a new table carries NO privileges for anyone, and `service_role` bypasses RLS but *not* table privileges — without the grant the Edge Function 500s with "permission denied for table movie_facts". Both directions are pinned in `rls_smoke.sql`. `scripts/warm_movie_facts.ts` (service-role + TMDB token) idempotently pre-fills it for already-journaled films.
